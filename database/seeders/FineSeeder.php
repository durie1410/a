...
class FineSeeder extends Seeder
{
    ...
    {
        // Lấy một số dữ liệu có sẵn
        $readers = Reader::limit(5)->get();
        $borrows = Borrow::with('borrowItems')->limit(10)->get();
        $users = User::limit(3)->get();

        if ($readers->isEmpty() || $borrows->isEmpty() || $users->isEmpty()) {
            $this->command->warn('Không có đủ dữ liệu (readers, borrows, users) để tạo phạt mẫu!');
            return;
        }

        $this->command->info('Đang tạo dữ liệu phạt mẫu...');
        ...
        if ($borrows->isNotEmpty()) {
            ...
            if ($borrowItem) {
                ...
                $this->command->info('✓ Đã tạo phạt trả muộn');
            }
        }

        // 2. Tạo phạt làm hỏng sách (có đầy đủ thông tin hư hỏng)
        if ($borrows->count() > 1) {
            $borrow = $borrows->skip(1)->first();
            $borrowItem = $borrow->borrowItems->first();
            
            if ($borrowItem && $borrowItem->inventory) {
                Fine::create([
                    'borrow_id' => $borrow->id,
                    'borrow_item_id' => $borrowItem->id,
                    'reader_id' => $borrow->reader_id,
                    ...
                    'condition_before' => $borrowItem->inventory->condition ?? 'Tot',
                ...
                $this->command->info('✓ Đã tạo phạt làm hỏng sách (có thông tin hư hỏng)');
            }
        }
        ...
        if ($borrows->count() > 2) {
            ...
            if ($borrowItem) {
                ...
                $this->command->info('✓ Đã tạo phạt mất sách');
            }
        }

        // 4. Tạo phạt đã thanh toán
        if ($borrows->count() > 3) {
            ...
            if ($borrowItem) {
                ...
                $this->command->info('✓ Đã tạo phạt đã thanh toán');
            }
        }
        ...
        if ($borrows->count() > 4) {
            ...
            if ($borrowItem) {
                ...
                $this->command->info('✓ Đã tạo phạt quá hạn');
            }
        }

        // 6. Tạo phạt hư hỏng nặng
        if ($borrows->count() > 5) {
            ...
            if ($borrowItem && $borrowItem->inventory) {
                Fine::create([
                    ...
                    'condition_before' => $borrowItem->inventory->condition ?? 'Moi',
                    'condition_after' => 'Hong',
                ...
                $this->command->info('✓ Đã tạo phạt hư hỏng nặng');
            }
        }

        $total = Fine::count();
        $this->command->info("\n✓ Hoàn thành! Đã tạo tổng cộng {$total} phạt mẫu.");
        $this->command->info('Bạn có thể vào trang quản lý phí phạt để xem dữ liệu.');
    }
}
